...
func (a *Application) Run(ctx context.Context) {
	...
	if numWorkers <= 0 {
		...
		if numWorkers > len(processedWallets) {
			...
			a.log.Info("Запрошено больше воркеров, чем кошельков, используется количество кошельков.",
				"requested", a.cfg.Concurrency.MaxParallelWallets, "using", numWorkers)
		}
		a.log.Info("Начало обработки кошельков в параллельном режиме",
			"count", len(processedWallets), "workers", numWorkers)
	}
...
func (a *Application) runSequentially(ctx context.Context, walletsToProcess []*wallet.Wallet) {
	...
	for i, w := range walletsToProcess {
		...
		if originalIndex == -1 {
			a.log.Error("Не удалось найти оригинальный индекс для кошелька, пропускаем.",
